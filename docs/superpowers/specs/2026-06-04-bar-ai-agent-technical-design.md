...
为提升系统在真实门店中的可用性、可控性和上线成功率，建议在核心业务闭环之外补充以下增强能力。

核心能力：

- 角色权限管理：按店长、采购、仓管、前台、运营等角色控制菜单、数据和操作权限。
- 操作审计：记录采购、库存调整、损耗、存酒核销、异常处理等关键操作的操作人、时间和变更内容。
- 扫码入库、出库和存酒核销：支持商品条码、二维码或存酒凭证码，提升仓库和前台操作效率。
- 库存批次与有效期管理：记录批次号、生产日期、到期日期和供应商来源，用于临期预警和库存清理。
- 客户存酒凭证与通知：存酒后生成二维码凭证，并支持到期前短信、微信或人工跟进提醒。
- 销售数据导入：支持 Excel 模板导入或 POS 数据对接，为补货建议和积压分析提供可靠消耗数据。
- 毛利与售价分析：结合采购成本、销售价格和活动折扣，判断哪些商品适合促销且不影响利润底线。
- 审批流：对高金额采购、库存报损、异常存酒处理和大额库存调整设置审批。
- 移动端轻量页面：面向仓库和前台提供扫码、入库、出库、盘点、取酒核销等高频操作入口。
- 异常工单：将库存差异、客户存酒争议、供应商延迟和商品破损形成可跟踪处理闭环。
...
随着系统从 MVP 进入正式运营阶段，建议增加以下扩展数据表，支撑权限、审计、批次、通知和审批能力。

| 表名 | 说明 |
| --- | --- |
| users | 系统用户表，记录账号、姓名、手机号、状态和所属角色 |
| roles | 角色表，维护店长、采购、仓管、前台、运营等角色 |
| role_permissions | 角色权限表，控制菜单、接口和关键操作权限 |
| audit_logs | 操作审计表，记录关键业务数据的新增、修改、删除和审批动作 |
| product_batches | 商品批次表，记录批次号、生产日期、到期日期、供应商和剩余库存 |
| storage_pickup_records | 客户取酒记录表，沉淀每次取酒核销明细 |
| customer_notifications | 客户通知记录表，记录存酒到期、活动召回和通知状态 |
| approval_requests | 审批申请表，记录采购、报损、库存调整和异常处理审批流程 |
| import_tasks | 数据导入任务表，记录 Excel 或 POS 数据导入批次、结果和错误信息 |
| exception_tickets | 异常工单表，记录库存差异、客户争议、供应商延迟和处理结果 |
...
| `/api/audit-logs` | 关键操作审计查询 |
| `/api/batches` | 商品批次、有效期和临期查询 |
| `/api/imports` | Excel 导入、POS 数据导入和导入结果查询 |
| `/api/approvals` | 审批申请、审批通过、驳回和审批历史 |
| `/api/exception-tickets` | 异常工单创建、分派、处理和关闭 |
| `/api/notifications` | 存酒到期提醒、客户召回通知和通知记录 |
...
| 移动端操作台 | 扫码入库、扫码出库、扫码盘点、客户取酒核销 |
| 审批中心 | 高金额采购、库存报损、库存调整和异常存酒审批 |
| 异常工单 | 库存差异、客户争议、供应商延迟、商品破损处理闭环 |
...
- 支持权限、审批流和操作审计。
- 支持异常工单闭环和跨岗位处理协同。
- 支持活动效果复盘，将活动前后销量、毛利和库存变化反馈给 Agent。
